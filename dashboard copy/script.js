...
function openEventModal(event) {
  ...
  const existingLocationElement = document.querySelector('.modal-location');
  if (existingLocationElement) {
    existingLocationElement.remove();
  }

  if (event.location && event.location.coordinates) {
    const locationElement = document.createElement('div');
    locationElement.className = 'detail-item modal-location';
    locationElement.innerHTML = `
      ...
      <span class="detail-value">
        ...
        ${event.location.placeName || `${event.location.coordinates.lat.toFixed(4)}, ${event.location.coordinates.lng.toFixed(4)}`}
      </span>
    `;
    document.querySelector('.event-details').appendChild(locationElement);
  }
